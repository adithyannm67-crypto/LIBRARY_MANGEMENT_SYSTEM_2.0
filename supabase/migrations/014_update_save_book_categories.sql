-- Update save_book RPC to save the category into the categories table FIRST,
-- then map it onto the books row via books.category_id.
-- Migration 013 already created save_book; this adds the category_id column,
-- backfills existing rows, and re-creates the RPC with the new flow.
-- Idempotent: safe to run on the live DB or a fresh rebuild.

-- ============================================================
-- 1. Ensure the books table can hold the category mapping
-- ============================================================
ALTER TABLE books ADD COLUMN IF NOT EXISTS category_id TEXT;

-- ============================================================
-- 2. Ensure write access for categories (RPC may create a category)
-- ============================================================
DO $$
BEGIN
...
-- ============================================================
-- 3. Backfill categories + books.category_id for existing books
--    (creates a categories row per distinct books.category name)
-- ============================================================
DO $$
DECLARE
  v_category_id TEXT;
  r RECORD;
BEGIN
  FOR r IN SELECT DISTINCT btrim(category) AS cat_name FROM books WHERE btrim(category) <> '' LOOP
    SELECT category_id INTO v_category_id
    FROM categories
    WHERE lower(name) = lower(r.cat_name)
    LIMIT 1;
    ...
    IF v_category_id IS NULL THEN
      ...
      VALUES (v_category_id, r.cat_name, 0)
      ON CONFLICT (name) DO NOTHING;

      -- re-read in case the unique(name) conflict won
      SELECT category_id INTO v_category_id
      FROM categories
      WHERE lower(name) = lower(r.cat_name)
      LIMIT 1;
    ...
    UPDATE books SET category_id = v_category_id
    WHERE lower(btrim(category)) = lower(r.cat_name) AND category_id IS NULL;
  END LOOP;
END $$;

-- ============================================================
-- 4. save_book RPC — the modal calls supabase.rpc('save_book', {...})
...
BEGIN
  ...
  -- 4. Resolve author by name; reuse if exists
